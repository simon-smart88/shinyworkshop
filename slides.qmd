...
```{r}
leafletOutput("map2")
numericInput("thresh1", "Threshold", value = 100)
```

```{r}
#| context: server

pal <- leaflet::colorBin("Greens", domain = c(0:100), na.color = "pink")

output$map2 <- leaflet::renderLeaflet({
              leaflet::leaflet() %>%
              leaflet::addProviderTiles("Esri.WorldTopoMap") %>%
              leaflet::addRasterImage(terra::clamp(fcover, input$thresh1, value=FALSE), colors = pal) %>%
              leaflet::addPolygons(data = wards, color = "black", weight = 1) %>%
...
```{r}
leafletOutput("map3")
numericInput("thresh2", "Threshold", value = 100)
```
...
observe({
  ...
  proxy_map %>%
    leaflet::addRasterImage(terra::clamp(fcover, input$thresh2 , value = FALSE), colors = pal) 
})
...
## Applications vs. scripts
-   Try and consider all the possibile ways a user could interact with your app and manage them
