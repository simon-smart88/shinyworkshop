...
- The server function takes two lists as arguments:
...
- Unlike in a normal R script, code isn't necessarily executed from top to bottom
:::
::: {.column width="30%"}
```{mermaid}
%%| fig-width: 6
...
- If you don't already, use the F1 key to look up documentation
...
-   These functions can be used to generate UI elements containing values generated from other inputs
-   UI:
```{r echo=TRUE, eval=FALSE}
...
- Placed at the top of reactive functions
...
## Software vs. scripts 
- Software has users and users can break things
- What happens if write this, but a user uploads a `.jpeg` instead of `.csv`?
...
df <- reactive({req(input$file)
                ...
                read.csv(input$file$datapath))  
                  ...
                  showModal(modalDialog(title = "Warning", "Please upload a .csv file"))
                }
                })
...
- `renderLeaflet()` and `leafletOutput()`
...
- Checks what R version you are using
- Looks at the calls to `library()` to establish which packages are used and their versions
- Replicates the environment of your machine. 
